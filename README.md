...
3. User Personas
•	The Risk Officer: Needs to see "Red Flags" (High Risk emails) immediately on a dashboard.
•	The Support Agent: Needs to see the sentiment of a customer and get a drafted reply instantly.
•	The Manager: Needs a high-level view of team communication health.
...
•	Technology: Python TextBlob + Custom Dictionary (keywords.py).
•	Function: Calculates Risk Score (0-100), Sentiment, and Tone.
•	Why: Zero latency, works offline, ensures privacy for sensitive detection.
•	Data Sources: Proprietary Enterprise Dictionary (Legal, HR, Finance domains).

B. Layer 2: The Creative System (Cloud / Online)
•	Technology: Google Gemini 2.0 Flash API.
•	Function: Context-aware drafting.
•	Why: Reads conversation history to generate human-like, empathetic responses that standard templates cannot match.
5. Functional Requirements
5.1 Authentication & Onboarding
•	Requirement: Users must sign up with valid enterprise emails.
•	Feature: Real-time DNS Validation (checks if the domain actually exists).
•	Feature: Secure Login/Logout with session management.
5.2 The Dashboard (Command Center)
•	Requirement: A single view for all incoming communications.
•	Feature: Traffic Light System:
o	🔴 Red: Risk Score > 70 (High Risk)
o	🟡 Yellow: Risk Score 51-70 (Neutral/Warning)
o	🟢 Green: Risk Score ≤ 50 (Safe)
•	Feature: Accordion View: Users can click a row to expand the full analysis card without leaving the dashboard.
•	Feature: Real-Time Sync: "Sync Gmail" button fetches emails via IMAP and auto-triggers analysis.
5.3 The Intelligence Engine
•	Requirement: Automated analysis upon email receipt.
•	Feature: Context Linking: The system retrieves the last 3 emails between the sender/recipient to detect "Hidden Patterns" (e.g., escalating anger).
•	Feature: Risk Scoring: Mathematical calculation based on weighted keywords (e.g., "Sue" = +30 risk).
5.4 Composition & Response
•	Requirement: Efficient email writing.
•	Feature: Smart Reply: "Reply with AI Draft" button pre-fills the response generated by Gemini.
•	Feature: Auto-Complete: User search bar filters names as you type.
•	Feature: Read Receipts: Unread emails appear Bold with a Blue Dot; marking as read happens on open.
